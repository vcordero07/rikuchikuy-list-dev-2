...
exports.createShort = (req, res) => {
  ...
  if (!longUrl) {
    return res
      .status(400)
      .json({ success: false, message: "You must provided a url!" });
  } else if (!isURL(longUrl)) {
    return res
      .status(400)
      .json({ success: false, message: "You must provided a valid url!" });
  }
    ...
    .then(url => {
      ...
      if (!url) {
        return res.redirect("/").json({
          success: false,
          message: "This url not exist in the system"
        });
      }
  ...
  Url.find({ longUrl: { $not: /porn/ } })
    ...
    .then(
      ...
      () =>
        res
          .status(500)
          .json({ success: false, error: "Something go wrong in the server!" })
    );
